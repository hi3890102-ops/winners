...
// v5 review (V5-2): an expense-ratio limit that could not be read is "기준 조회 실패 / 판정 불가" on EVERY screen that uses it — never
// green — while a normally unset limit (NULL) is the 40% default. Checked on the real render output of the owner home, the owner's
// monthly report and the manager home card, with the same store data and the same limit states.
// 2026-09-22 owner decision: 지출비율 = TOTAL expense ÷ sales (every category, classification never affects this ratio) - reverted
// from the earlier "confirmed 식자재 only + pending while unclassified" design; see expense-category-and-vat.test.mjs for the
// category-split utility itself, which is unrelated to this ratio now.
...
function screens(limitState,{sales=1000000,expense=600000}={}){
  const state={role:'storeOwner',store:'A',myStores:['A'],monthYear:2026,monthNum:9,storeFoodLimitMap:limitState===undefined?{}:{A:limitState},
    ...
    expenseEntries:expense?[{amount:expense,category:null}]:[],
    dashboardLoading:false,storeList:['A'],dashboardDailyTrend:{},dashboardTrendActiveDate:null};
  const ctx=vm.createContext({state,console,escapeHtml:s=>String(s),daysInMonth:()=>30,computeVendorBreakdown:()=>[{name:'거래처A',amount:expense}],calcCrewPay:()=>({pay:0,hours:0,days:0}),
  ...
  const dashRow={store:'A',foodThreshold:failure?null:(typeof limitState==='number'?limitState:40),salesSum:sales,salesReportCount:1,deliverySum:0,laborPay:0,laborRatio:5,expenseSum:expense,expenseRatio:ratio,
    prevMonthSalesSum:0,prevMonthFailed:false,loadFailures:failure?['지출 기준']:[],dailySales:{},crewCount:0};
...
test('Same data (sales 1,000,000 / expenses 600,000 = 60%), limit READ as unset: every screen says "over" (default 40%) — no green',()=>{
  ...
  assert.match(s.owner,/관리 필요[\s\S]*지출비율 40% 초과/);
  assert.match(s.report,/food-verdict" data-state="over"[\s\S]*?관리 필요/);
  assert.match(s.manager,/color:var\(--danger\)[^>]*>60\.0%/);
});
test('Limit NOT READABLE: monthly report shows "기준 조회 실패 · 판정 불가", not "기준 이내"; manager home is neutral with the same wording; owner home is "확인 필요"',()=>{
  ...
  assert.equal(/지출비율[\s\S]{0,40}기준 이내/.test(s.report),false,'no "지출비율 ... 기준 이내"');assert.equal(/#146C3A/.test(s.report.match(/<div class="food-verdict"[\s\S]*?<\/div>/)[0]),false,'and no green colour on the chip');
  ...
  assert.match(s.owner,/확인 필요|읽지 못한 항목: 지출 기준/);assert.equal(/합산 기준 안정/.test(s.owner),false);
});
test('Limit read as a saved value: every screen uses it (38% -> over at 60%, and at 38% exactly it is fine)',()=>{
  const over=screens(38);
  assert.match(over.owner,/지출비율 38% 초과/);assert.match(over.report,/data-state="over"/);assert.match(over.manager,/var\(--danger\)[^>]*>60\.0%/);
  const equal=screens(38,{sales:1000000,expense:380000});
  assert.match(equal.report,/food-verdict" data-state="ok"[\s\S]*?기준 이내/);assert.match(equal.manager,/var\(--olive\)[^>]*>38\.0%/);assert.equal(/지출비율 38% 초과/.test(equal.owner),false);
});
...
test('No ratio (no sales): no expense verdict is shown at all, on any screen',()=>{
  const state=screens(false,{sales:0,expense:0});
  assert.equal(/지출비율[\s\S]{0,10}(기준 이내|관리 필요)/.test(state.report),false);
});
