// Rollback target for the staging `manee-staff-signup` Edge Function.
// = main 2e4e162 v1 (sha256 caa30ef9…, identical to the deployed staging v1) + ONE guard block.
// Deploy THIS file, not the plain v1, when rolling back new-staff-self-profile.
// Deploy as slug manee-staff-signup with verify_jwt=false (same as the current deployment).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ROLLBACK GUARD (not in the original v1): a newer signup form may still be open in a
    // browser tab after the rollback. It sends `personal` (phone/bank), which this older
    // function cannot store. Fail loudly BEFORE any rate-limit use or account creation.
    if (body && typeof body === 'object' && Object.hasOwn(body, 'personal')) {
      return json({ error: 'app_update_required', message: '앱이 업데이트되어 가입 정보를 저장할 수 없어요. 화면을 새로고침한 뒤 다시 가입해 주세요. (입력하신 정보는 저장되지 않았어요)' }, 426)
    }
