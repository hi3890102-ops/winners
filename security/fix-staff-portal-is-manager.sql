-- Hotfix: the owner's "매니저 권한 부여" checkbox in crew management only ever
-- updated crew.is_manager, but the live staff-session RPC's 'crew' action
-- computed is_manager purely from store_memberships.role -- a field no code
-- path ever sets to 'manager'. Net effect: since production cutover, no
-- manager checkbox toggle has actually taken effect for any staff-auth
-- account (스텝 관리 tab, manager checklist controls, etc. never appear).
-- This is a single-expression fix, extracted and verified on staging first.
CREATE OR REPLACE FUNCTION private.manee_staff_portal_v1(p_action text, p_payload jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if p_action='session' then
    ...
    return jsonb_build_object('ok',true,'crew',(to_jsonb(person)-array['resident_number','bank_account','notes','join_code']) || jsonb_build_object('is_manager',coalesce(person.is_manager,false) or existing_role='manager'));
