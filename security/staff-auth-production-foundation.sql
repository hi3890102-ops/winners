...
begin
  ...
  if p_action='session' then
    ...
    if not public.consume_auth_rate_limit('staff_link_user',md5(actor::text),300,10) then return jsonb_build_object('ok',false,'error','rate_limited'); end if;
    ...
    if not found or (person.resign_date is not null and person.resign_date <= (now() at time zone 'Asia/Seoul')::date) then return jsonb_build_object('ok',false,'error','invalid_code'); end if;
    ...
    if exists(select 1 from public.store_memberships where crew_id=person.id and user_id<>actor) then return jsonb_build_object('ok',false,'error','record_already_linked'); end if;
    ...
    if (select count(*) from private.staff_link_requests where requester_user_id=actor and status='pending')>=10 then return jsonb_build_object('ok',false,'error','too_many_pending'); end if;
