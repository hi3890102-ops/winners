...
with checks(name,ok) as (
  ...
  union all select 'public RPCs: authenticated yes, anon no', (has_function_privilege('authenticated','public.manee_my_profile_state(boolean)','execute') and has_function_privilege('authenticated','public.manee_save_my_profile(jsonb,integer)','execute') and has_function_privilege('authenticated','public.manee_owner_profile_changes(uuid,integer)','execute')
     and not has_function_privilege('anon','public.manee_my_profile_state(boolean)','execute') and not has_function_privilege('anon','public.manee_save_my_profile(jsonb,integer)','execute') and not has_function_privilege('anon','public.manee_owner_profile_changes(uuid,integer)','execute'))
  union all select 'sync helper not callable by clients', (not has_function_privilege('authenticated','private.sync_self_profile_to_crew(uuid,uuid,text)','execute') and not has_function_privilege('anon','private.sync_self_profile_to_crew(uuid,uuid,text)','execute'))
