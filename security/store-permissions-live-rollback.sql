...
begin
  ...
  perform set_config('request.jwt.claims',jsonb_build_object('session_id',owner_id)::text,true);
  execute 'set local role authenticated';
  assert (select count(*) from public.stores)=1,'store list leaked another store';
  blocked:=false;
  begin perform value from public.app_settings; exception when insufficient_privilege then blocked:=true; end;
  assert blocked,'shared PIN settings exposed';
  blocked:=false;
  begin update public.stores set owner_username='not-allowed' where id=sid; exception when insufficient_privilege then blocked:=true; end;
  assert blocked,'client changed protected ownership metadata';
  result:=public.manee_checklist('initialize',sid,'2026-01-15',
  ...
  perform set_config('request.jwt.claims',jsonb_build_object('session_id',owner_id)::text,true);
  execute 'set local role authenticated';
  assert (public.manee_staff_portal('approve',jsonb_build_object('request_id',rid))->>'ok')::boolean,'reactivation failed';
  execute 'reset role';
  perform set_config('request.jwt.claim.sub',worker_id::text,true);
  perform set_config('request.jwt.claims',jsonb_build_object('session_id',fresh_session)::text,true);
  execute 'set local role authenticated';
  result:=public.manee_checklist('check',sid,((now() at time zone 'Asia/Seoul')-interval '6 hours')::date,'{"item_id":"clean","checked":true}'::jsonb);
  assert (result->'checks'->>'clean')::boolean,'checkbox failed';
  result:=public.manee_checklist('check',sid,((now() at time zone 'Asia/Seoul')-interval '6 hours')::date,'{"item_id":"lights","checked":true}'::jsonb);
  assert (result->'checks'->>'clean')::boolean and (result->'checks'->>'lights')::boolean,'checkbox replaced other checks';
  result:=public.manee_checklist('close',sid,((now() at time zone 'Asia/Seoul')-interval '6 hours')::date,'{"done":999,"total":999}'::jsonb);
  assert (result->'log'->>'done')::int=2 and (result->'log'->>'total')::int=2,'client forged close totals';
  blocked:=false;
  begin perform public.manee_checklist('reopen',sid,((now() at time zone 'Asia/Seoul')-interval '6 hours')::date);
  exception when insufficient_privilege then blocked:=true; end;
  assert blocked,'employee reopened a closed day';
  insert into public.reservations(store_id,date,customer_name,created_by) values(sid,'2026-01-15','Synthetic guest','Forged boss');
  assert (select count(*) from public.reservations where store_id=sid and created_by='Forged boss')=0,'forged reservation author';
  assert (select count(*) from public.sales_reports)=0,'staff obtained unassigned financial data';
  execute 'reset role';
  execute 'set local role anon';
  blocked:=false;
  begin perform id from public.reservations; exception when insufficient_privilege then blocked:=true; end;
  assert blocked,'anonymous reservation access';
  execute 'reset role';
