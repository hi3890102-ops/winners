-- Production staff Auth foundation — PART 1 (safe regardless of who is currently clocked in).
-- Split from staff-auth-production-foundation.sql on 2026-09-16 because that file also replaces
-- public.clock_in/public.clock_out, which are live functions real staff use to clock in/out today.
-- This part creates everything else: new tables, helper functions, manee_staff_portal, the
-- one-open-attendance-per-crew index, and the new private.staff_clock implementation — but does
-- NOT wire public.clock_in/public.clock_out to it yet. Apply this anytime.
-- See staff-auth-production-foundation-part2-clock-deferred.sql for the remaining, time-sensitive part.
begin;
...
$$;
-- NOTE: public.clock_in / public.clock_out are intentionally NOT wired to private.staff_clock here.
-- Apply staff-auth-production-foundation-part2-clock-deferred.sql once nobody has an open attendance row.
revoke all on function private.staff_clock(uuid,uuid,double precision,double precision) from public,anon;
grant execute on function private.staff_clock(uuid,uuid,double precision,double precision) to authenticated;
