-- Production support password recovery foundation.
-- Additive migration: existing profiles, memberships, crew and business data are preserved.
-- Initial platform administrator assignment is intentionally NOT performed here.
begin;
...
-- Some staging histories have a legacy key table; current production does not.
-- If it exists in a future environment, invalidate old keys without requiring it.
do $retire_legacy_keys$
begin
  if to_regclass('private.account_recovery_keys') is not null then
    execute 'update private.account_recovery_keys set key_hash=null where key_hash is not null';
  end if;
end;
$retire_legacy_keys$;
